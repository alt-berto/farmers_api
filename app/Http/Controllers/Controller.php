...
class Controller extends BaseController
{
    ...
    protected function invitation_mail(int $order_id): string {
        try {
            $order = Order::with(['details.inventory_price.inventory.product'])->where('state', 'done')->where('id', $order_id)->firstOrFail();
            $user = User::where('id', $order->client_id)->firstOrFail();
